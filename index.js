...
trackerIds.forEach(function (trackerId, index) {
    ...
    listeners[index] = new gpsd.Listener({
        port: 8085+index,
        hostname: 'localhost',
        logger: {
            info: function () {
    ...
    listeners[index].connect(function () {
        console.log('listener connect '+index);
    });
//gpsd events like described in the gpsd documentation. All gpsd events like: TPV, SKY, INFO and DEVICE can be emitted. To receive all TPV events just add
    ...
    listeners[index].on('TPV', function (data) {
        //console.log(data);
    ...
    listeners[index].on('SKY', function (data) {
        //console.log(data);
    });

    listeners[index].on('INFO', function (data) {
        //console.log(data);
    });

    listeners[index].on('DEVICE', function (data) {
        //console.log(data);
    });

    listeners[index].watch({class: 'WATCH', json: true, nmea: false});
});
